<%@page import="cn.smbms.util.PageUtil"%>
<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@include file="/WEB-INF/jsp/common/head.jsp"%>
<%
	if (request.getAttribute("proPage") == null){
		
		response.sendRedirect("provider.html?method=query");
	} else {
		
		PageUtil pUtil = (PageUtil)request.getAttribute("proPage");
		request.setAttribute("proPage", pUtil);
	}
...
<div class="right">
        ...
        <div class="search">
        	<form name="formList" method="get" action="${pageContext.request.contextPath }/provider.html?method=query">
				<input name="pageIndex" value="" type="hidden">
				...
				<a href="${pageContext.request.contextPath }/toAddPro.html">添加供应商</a>
			</form>
        </div>
        <!--供应商操作表格-->
        <table class="providerTable" cellpadding="0" cellspacing="0">
            ...
            <c:forEach var="provider" items="${proPage.proList }" varStatus="status">
				<tr>
					<td>
		...
		<input type="hidden" id="totalPageCount" value="${proPage.total}"/>
		  	<c:import url="rollpage.jsp">
	          	<c:param name="totalCount" value="${proPage.count}"/>
	          	<c:param name="currentPageNo" value="${proPage.index}"/>
	          	<c:param name="totalPageCount" value="${proPage.total}"/>
          	</c:import>
    </div>
</section>
